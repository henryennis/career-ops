...
| 2. Additive files | New scripts, fork-only modes, a CV template, a scan provider, a CLI prompt pack | `fork/` for anything upstream does not discover by filename. A provider goes in `providers/<name>.mjs` and a template in `templates/cv-template.<name>.html` (plus its fonts in `fonts/`) because upstream loads those directories by glob. Anything outside upstream's system directories is declared in `fork/local-paths.txt`. | Near none. A file upstream does not have cannot conflict. It can break when an upstream module it imports changes shape; the test run on every sync catches that. |
...
| SEEK Australia as a scan source | 1 | A `portals.yml` entry with `provider: jobstreet`, `api` pointing at `www.seek.com.au`'s v5 search endpoint (the provider already allows that host) and an Australian `siteKey` (probably `AU-Main`; the provider's comment table lists only ID, SG, MY and HK, so confirm with one scan) |
| STAR story bank | 0 | `interview-prep/story-bank.md` under the data root |
| Inbox refilter after a targeting change | 2 | `fork/refilter.mjs`, importing upstream's title filter, tier classifier, location and posting-age filters so it cannot disagree with `scan.mjs`; a `fork/modes/refilter.md` for the agent |
...
| Work Sans monochrome CV | 2 | `templates/cv-template.worksans.html` wired through `build-cv-html.mjs` placeholders, `fonts/work-sans-*.woff2`, and `cv.template: worksans` in `config/profile.yml` |
